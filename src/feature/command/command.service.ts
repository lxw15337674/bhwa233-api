...
import {
  getStockData as getStockNewData,
  getStockDetailData as getStockDetailNewData,
} from './acions/stock';
import { getHotSpot } from './acions/stockHotSpot';
import {
  getCNMarketIndexData,
  getHKMarketIndexData,
  getStockData,
  getStockDetailData,
  getStocksByTag,
  getAllStockGroups,
  getUSMarketIndexData,
} from './acions/stockInfo';
...
export class CommandService {
        ...
        const response = await this.aiService.generateResponseWithTools(
          {
            prompt,
            rolePrompt:
              '你是坤哥，你会为用户提供安全，有帮助，准确的回答，不要回答中国政治相关内容，回答控制在200字以内。回答开头是：坤哥告诉你，结尾是：厉不厉害 你鸡哥🐔',
          },
          {
  ...
  async executeCommand(
    ...
    for (const command of this.commandMap) {
      ...
      if (isMatch) {
        const args = command.hasArgs
          ? msg.slice(command.key.length).trim()
          : undefined;
  ...
  private getAiCommandTools(): {
    ...
    tools.push({
      ...
      function: {
        name: tavilyToolName,
        description:
          '搜索互联网最新信息。适合时效性问题、新闻、公告、产品更新和事实核验。',
        parameters: {
          type: 'object',
          properties: {
            query: {
    ...
    extraToolMap.set(tavilyToolName, async (args: unknown) => {
      const query =
        typeof (args as { query?: unknown })?.query === 'string'
          ...
          : '';
      const maxResults =
        typeof (args as { maxResults?: unknown })?.maxResults === 'number'
  ...
  async getRelayPulseScreenshot(
    ...
    return await takeRelayPulseScreenshot(
      this.screenshotService,
      provider,
      period,
    );
