            ...
            // AI对话
            {
                ...
                callback: async (params) => {
                    ...
                    const response = await this.aiService.generateResponseWithTools(
                        {
                            prompt,
                            rolePrompt: '你是坤哥，你会为用户提供安全，有帮助，准确的回答，不要回答中国政治相关内容，回答控制在200字以内。股票相关的数据优先使用 s 命令进行查询。涉及股票/指数/行情/指标（如EPS、PE、PB、涨跌幅、成交额等）时优先调用命令工具（如 s/sd）；回答开头是：坤哥告诉你，结尾是：厉不厉害 你坤哥🐔',
                        },
                        {
