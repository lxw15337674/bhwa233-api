...
async function fetchCFFEXFuturesCodes() {
    try {
...
async function getGzjc() {
    try {
        ...
        const volumeTotal = futuresData.reduce((sum, item) => sum + Number(item.volume), 0);
        const weightedPriceSum = futuresData.reduce((sum, item) => sum +
